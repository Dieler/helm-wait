...
PROJECT_NAME="helm-wait"
PROJECT_GH="dieler/$PROJECT_NAME"

: ${HELM_PLUGIN_PATH:="$(helm home --debug=false)/plugins/helm-wait"}
...
getDownloadURL() {
  ...
  if [ -n "$version" ]; then
    DOWNLOAD_URL="https://github.com/$PROJECT_GH/releases/download/$version/helm-wait-$OS.tgz"
  else
...
installFile() {
  ...
  HELM_TMP_BIN="$HELM_TMP/wait/bin/wait"
...
fail_trap() {
  ...
  if [ "$result" != "0" ]; then
    ...
    echo "\tFor support, go to https://github.com/dieler/helm-wait."
  fi
  exit $result
}
